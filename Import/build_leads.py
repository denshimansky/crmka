...
⚠️ Скрипт — исторический прототип веб-этапа 1 («Настройки → Импорт базы»).
   Каноническая версия — app/src/lib/leads-import/process-leads.ts: она дополнительно
   понимает полное ФИО в «Контактном лице» (берёт ячейку целиком) и конфликт
   филиалов. Логика здесь НЕ обновляется — для реальных импортов используйте CRM.
