...
> Обновлено 19.07.2026: портал клиента v2 (парольный вход `/api/portal/login`, учётка `portal-account`, согласия `consents`, контакты филиалов `portal-contacts`).
...
- [Учётная запись портала клиента](#учётная-запись-портала-клиента)
...
- [Контакты филиала для портала (Portal Contacts)](#контакты-филиала-для-портала-portal-contacts)
...
| `instructor` | Инструктор/инструктор — ограниченный доступ |
...
## Учётная запись портала клиента

> Заменила «Ссылку на портал клиента» (v2, 19.07.2026). `/api/clients/[id]/portal-link` **удалён** — магические ссылки отключены.

| Метод | Путь | Авторизация | Описание |
|-------|------|-------------|----------|
| GET | `/api/clients/[id]/portal-account` | Да, право `clients.edit` | Состояние учётки ЛК клиента (есть/нет, активность, телефон-логин, дата выдачи пароля) |
| POST | `/api/clients/[id]/portal-account` | Да, право `clients.edit` | Выдать учётку / перевыпустить пароль. Пароль (`xxxx-xxxx`) возвращается **один раз** + готовое сообщение родителю. Перевыпуск завершает все сессии |
| DELETE | `/api/clients/[id]/portal-account` | Да, право `clients.edit` | Деактивировать учётку (завершает все сессии) |

**Ошибки POST:**
- `422` — телефон клиента отсутствует или невалиден (нужен канон 11 цифр «7…», только РФ)
- `409 PHONE_DUPLICATE` — телефон дублируется у другого клиента организации (сначала почистить `/crm/duplicates`)
- `422 DOCS_MISSING` — не заполнены 4 обязательных URL документов в настройках организации
...
> **v2 (19.07.2026).** Сессия — JWT v2 в cookie `portal-token` (30 дней, `{v:2, clientId, tenantId, slug, clientName}`), инвалидируется перевыпуском пароля/деактивацией (`iat >= passwordIssuedAt`). Все эндпоинты данных проверяют активность учётки и обязательные согласия по БД; без согласий — `403 CONSENTS_REQUIRED`.

| Метод | Путь | Авторизация | Описание |
|-------|------|-------------|----------|
| POST | `/api/portal/login` | Нет (rate-limit: IP 5/15 мин + телефон 10/15 мин) | Вход: `{ slug, phone, password }`. Единый ответ на любую ошибку — «Неверный телефон или пароль» |
| GET | `/api/portal/auth` | Portal JWT | Проверка сессии портала |
| DELETE | `/api/portal/auth` | Portal JWT | Выход из портала |
| GET | `/api/portal/consents` | Portal JWT | Состояние согласий + список документов организации (для гейта) |
| POST | `/api/portal/consents` | Portal JWT | Записать согласия (append-only журнал ClientConsent; обязательные отозвать нельзя, опциональные — можно) |
| GET | `/api/portal/me` | Portal JWT + согласия | Обзор: профиль, подопечные, абонементы, ближайшие занятия (14 дней) + пробные, баланс, контакты филиала |
| GET | `/api/portal/wards/[wardId]/attendances?offset=` | Portal JWT + согласия | Посещения подопечного: сводка + список (пагинация) |
| GET | `/api/portal/wards/[wardId]/timeline` | Portal JWT + согласия | История подопечного: лента событий (абонементы, отчисления, пробные) |
| GET | `/api/portal/payments?offset=` | Portal JWT + согласия | Оплаты клиента: баланс + список со способами (пагинация) |

**Удалены в v2:** `POST /api/portal/auth?token=` (вход по магической ссылке), `GET /api/portal/data`, `POST /api/portal/consent`, а также `/api/clients/[id]/portal-link`. Старые ссылки `/portal?token=...` редиректят на `/p/<слаг>?from=legacy`.

---

## Контакты филиала для портала (Portal Contacts)

| Метод | Путь | Авторизация | Описание |
|-------|------|-------------|----------|
| PATCH | `/api/branches/[id]/portal-contacts` | owner, manager | Контакты филиала для ЛК родителя: `contactPhone`, `contactWhatsapp`, `contactTelegram`, `contactMax` |
