...
| **Аутентификация** | NextAuth.js v5 | JWT, multiple providers (см. раздел 5) |
...
| Способ | Приоритет | Для кого | Реализация |
|--------|-----------|----------|------------|
| **Telegram OAuth** | MVP | Все (основной способ в РФ) | NextAuth provider, Telegram Login Widget |
| **Яндекс OAuth** | MVP | Все (второй по популярности) | NextAuth provider, Яндекс ID |
| **SMS-код** | MVP | Инструкторы, администраторы (нет email/соцсетей) | SMS-провайдер (SMS.ru / SMSAero) |
| **Magic link (email)** | MVP | Владельцы, управляющие | NextAuth Email provider, SMTP Timeweb |
| **Google OAuth** | MVP | Опционально | NextAuth Google provider |
| **Логин + пароль** | v1.1 | Фолбэк | NextAuth Credentials provider |

### Двухфакторная аутентификация (2FA)

**Обязательна** для ролей owner и manager (доступ к финансам, зарплатам, P&L).

| Метод 2FA | Приоритет |
|-----------|-----------|
| SMS-код | MVP |
| Telegram-бот (push-подтверждение) | v1.1 |
| TOTP (Google Authenticator) | v2 |

Логика:
- При первом входе owner/manager — принудительная настройка 2FA
- admin/instructor — опционально (включается в настройках организации)
- 2FA запрашивается при: входе с нового устройства, смене пароля, критичных операциях (возврат, удаление данных, закрытие периода)

### Инвайт-система

1. Owner создаёт приглашение (email или ссылка)
2. Приглашённый переходит по ссылке, выбирает способ входа
3. Привязывается к организации с указанной ролью
4. Инвайт-ссылка одноразовая, с TTL (72 часа)

### Роли

| Роль | Описание | Примеры прав |
|------|----------|-------------|
| **owner** | Владелец/руководитель | Всё. Финрез, закрытие периодов, удаление данных, управление пользователями. 2FA обязательна |
| **manager** | Управляющий | Почти всё, кроме SaaS-настроек. CRM, финансы, зарплата, отчёты. 2FA обязательна |
