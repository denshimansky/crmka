...
|---|--------|---------|--------|
| 1 | Домен? (smartcrm.ru, crm.obrp.it, другой) | Деплой, SSL | Ждёт решения |
| 2 | Отдельный VPS или рядом с Life OS? | Инфра | Рекомендую отдельный |
| 3 | Формат миграции данных из 1С? (CSV, API, ручной) | Спринт 5 | Ждёт обсуждения |
| 4 | PWA или native приложение? | UX | Рекомендую PWA |
| 5 | Какой email-провайдер для magic link? (Resend, Mailgun) | Auth | Ждёт решения |
| 6 | Нужен ли белый лейбл (лого партнёра)? | UI | Ждёт решения |
