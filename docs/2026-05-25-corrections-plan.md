...
- **Матрица посещений**: 6 строк (Был, Не был, УП, Прогул, Перерасчёт, Отработка), 6 столбцов (доступно инструктору, План, Факт, Прогноз, Расчёт, Начисление ЗП). Настраивает владелец в settings.
...
- `availableToInstructor: Boolean @default(false)` — «доступно инструктору» (фильтрует выпадашку отметки в карточке занятия).
...
| Код | Название | Доступно инструктору | План | Факт | Прогноз | Расчёт | ЗП |
|---|---|:-:|:-:|:-:|:-:|:-:|:-:|
| `present` | Был | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| `no_show` | Не был | ✓ | ✓ | — | ✓ | ✓ | — |
...
## Фаза 3 — Ставки инструкторов (п.1 + п.2)
...
1. Если у группы есть `GroupSalaryRate` → берём её для ВСЕХ инструкторов (включая замену) — это полностью перекрывает их личные ставки.
...
4. Если нет — ЗП = 0, логируем предупреждение для отчёта «инструкторы без ставки».
...
- Карточка группы [app/src/app/(dashboard)/schedule/groups/edit-group-dialog.tsx](app/src/app/(dashboard)/schedule/groups/edit-group-dialog.tsx): новая секция «Ставка для группы (перебивает ставки инструкторов)», по умолчанию пусто.
...
2. Проверка конфликта (инструктор/кабинет уже занят) → возвращаем 409 с деталями; фронт показывает модалку подтверждения.
...
3. **Плавающая ставка и ставка группы**. Если в группе задана фиксированная ставка `per_lesson=500₽`, а у инструктора была плавающая по матрице — ставка группы полностью перекрывает. То есть `GroupSalaryRate` поддерживает любую из 4 схем, не только число. Это уже учтено в 3.2.

4. **«Не был» как промежуточный статус**. По матрице — списывает с абонемента, ЗП не начисляет. Это рискованно: если оператор забыл перевести в финальный статус — посещение не оплачено инструктору. Mitigation: в задачи добавить триггер «занятия со статусом Не был старше 7 дней».

5. **Откат при переносе даты**. Если занятие было давно и по нему уже выплачена ЗП (`SalaryPayment` создан), откат `Attendance.instructorPayAmount` не вернёт деньги инструктору. Нужно либо блокировать перенос после выплаты, либо создавать `SalaryAdjustment(type=correction, amount=-X)`. Предлагаю **блокировать** перенос занятий из закрытого периода — это согласуется с концепцией «мягкого закрытия периода» (CLAUDE.md).
...
**Сценарий 2 — Ставки:**
1. Инструктор: плавающая матрица (1-3 → 400, 4 → 500, 5 → 600).
...
4. Замена инструктора без ставки → ЗП = 1000 (всё равно по группе).
