...
Ставка ЗП, привязанная к группе. **Перекрывает** личные `SalaryRate` всех инструкторов (включая замещающего) при расчёте ЗП за конкретное занятие.
...
| is_one_time | Boolean | да | Техническая группа под одно занятие вне расписания (разовое занятие). Скрыта из реестра групп и отчётов по группам | — |
| is_trial_holder | Boolean | да | Техническая группа-держатель индивидуального пробного (всегда вместе с is_one_time). Дополнительно скрыта из сетки расписания — там пробное рисуется своей карточкой | — |
| created_at | DateTime | да | Дата создания | — |
| updated_at | DateTime | да | Дата обновления | — |
| deleted_at | DateTime | нет | Мягкое удаление | — |
...
Вид дня — справочник (Был, Не был, Уваж. пропуск, Прогул, Перерасчёт, Назначена отработка, Отработано + пользовательские).
...
| available_to_instructor | Boolean | да | **Ф1:** Доступно инструктору в выпадашке отметки | — |
...
| withdrawal_affects_instructor | Boolean | нет | Отток по инструктору (для отчётов) | — |
...
| first_paid_lesson_date | Date | нет | Дата первого платного занятия. Агрегат уровня родителя = самая ранняя среди детей (min по Application.first_paid_lesson_date и первого платного посещения). Авто при посещении «в долг»; в воронке «Продаж» правится per-ребёнок на заявке (Application), а здесь пересчитывается | — |
...
| total_lessons | Int | да | Количество занятий в абонементе. **Для calendar следует за расписанием (13.07.2026):** при физическом добавлении/удалении занятий группы в диапазоне абонемента пересчитывается дельтой (`lib/subscriptions/recalc-on-schedule-change.ts`) вместе с суммами; при отложенном отчислении срезается по дату X (`prorate-scheduled-withdrawal.ts`). Отмена занятия (status=cancelled) НЕ декрементит (вариант A) | — |
...
>
> **Пробное без группы (индивидуальное):** `group_id` остаётся NULL, а `lesson_id` указывает на собственное техническое занятие в скрытой группе-держателе (`Group.is_trial_holder`). Занятие нужно для ЗП: начисление инструктору живёт только в `Attendance.instructor_pay_amount` у `Lesson`. Отметка «Был» создаёт `Attendance(is_trial=true, charge_amount=0)` — клиенту пробное бесплатно, педагогу считается по его ставке с учётом режима «Оплата за пробное занятие» (`SalaryRate.trial_pay_mode`) и галочки `instructor_pay_enabled` на самом пробном.
...
**Ф7:** `missed_makeup` создаётся, когда: (1) инструктор отметил «Не был» на виртуальной строке отработки; (2) целевое занятие отработки отменено. Задача назначается админу/менеджеру/владельцу — переназначить отработку.
