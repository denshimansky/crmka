 ...
 * отметка «Был» создаёт `Attendance(isTrial, chargeAmount = цена пробного или
 * 0, если направление «Бесплатное пробное»)` — см. lib/services/trial-charge.ts,
 * а `instructorPayAmount` считается через resolveRate/calcPay по личной ставке
 * инструктора с учётом режима «Оплата за пробное занятие».
 *
