...
export const TRIGGER_LABEL: Record<TaskAutoTrigger, string> = {
  ...
  payment_due: "Напомнить об оплате (заявка с обещанной датой)",
  no_show_review: "Уточнить «Не был» (со следующего дня)",
...
export const TRIGGER_DESCRIPTION: Record<TaskAutoTrigger, string> = {
  ...
  payment_due:
    "Подопечный в «Ожидаем оплату», и у клиента проставлена обещанная дата оплаты — напоминание к этой дате (без обещанной даты задача не создаётся)",
  no_show_review:
    "Отметка «Не был» вчера или раньше не уточнена (прогул / уваж. пропуск / отработка)",
