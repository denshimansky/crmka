 ...
 *
 * dueDate задачи = сегодня (день появления), а НЕ nextContactDate: задача должна
 * быть «актуальной сегодня», а не сразу «просроченной», если дата связи в прошлом
 * (крон пропустил день / дату выставили задним числом). Просрочка считается по
 * dueDate < сегодня, значит на следующий день задача сама станет просроченной.
 * Дедуп по dueDate ≥ nextContactDate по-прежнему ловит «тот же цикл» (today всегда
 * ≥ nextContactDate, раз сюда попали); новую (более позднюю) дату связи менеджер
 * задаст — её nextContactDate окажется больше dueDate прошлой задачи, и создастся
 * следующая.
 *
 * Узкий edge (принят осознанно): если задача была создана «с опозданием»
 * (dueDate=today > nextContactDate — крон пропустил дни / дата задним числом),
 * а потом менеджер задаёт новую дату связи ЗАДНИМ ЧИСЛОМ в окне (старая_дата,
 * день_создания], дедуп её погасит. Для будущих дат (обычный сценарий) регрессии
 * нет. Маркерный дедуп решил бы это, но вызвал бы всплеск дублей у существующих
 * задач без маркера — не стоит того ради редкого кейса.
 *
