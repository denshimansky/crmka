...
export function buildNavMap(): string {
  ...
  const faq = [
    ...
    "- Импорт клиентов / миграция базы (CSV/XLSX): левое меню → «Настройки» → вкладка «Персональные» → «Импорт базы» (доступно только владельцу). Кнопки «Импорт» на странице «Клиенты» больше НЕТ.",
