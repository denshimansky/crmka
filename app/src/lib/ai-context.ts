...
export function buildNavMap(): string {
  ...
  const faq = [
    ...
    "- Массовое продление абонементов на следующий месяц (выписать ВСЕМ одной кнопкой): «Абонементы» в левом меню (блок «CRM») → вкладка «Ожидающие оплаты» → кнопка «Выписать абонементы на следующий период» (доступно только владельцу и управляющему; учитывает текущие фильтры по филиалу/направлению). Откроется превью (сколько выпишется и на какую сумму, кто пропущен — уже продлённые, группы без расписания на период; дети с запланированным отчислением исключаются), затем подтверждение. Создаёт по абонементу-счёту в статусе «Ожидает оплаты» на каждый активный календарный абонемент: цена занятия берётся из АКТУАЛЬНОГО прайса направления (не из старого абонемента), число занятий пересчитывается по расписанию группы на новый месяц, скидки пересчитываются автоматически. Оплата проводится отдельно. Это и есть ответ на «как продлить всем сразу» — функция ЕСТЬ.",
    "- Продлить абонемент одному клиенту: карточка клиента → кнопка «Абонемент» в шапке (диалог «Продление абонемента»: копирует группу/направление/подопечного из текущего активного календарного абонемента, цена — по актуальному прайсу направления, создаёт абонемент-счёт «Ожидает оплаты» на следующий месяц). Альтернатива с ручным выбором группы: вкладка «Абонементы» в карточке → кнопка «Абонемент» (диалог «Новый абонемент»).",
    "- ВАЖНО про автопродление: автоматического создания абонементов на следующий месяц по расписанию НЕТ — выписывает человек (массово или поштучно, см. выше). Плитка «Настройки → вкладка Организация → Абонементы (модель + автозакрытие)» отвечает за модель абонементов (календарная/пакетная/фиксированная) и срок АВТОЗАКРЫТИЯ неоплаченных абонементов, а НЕ за автопродление (полностью отработанные календарные абонементы без долга закрываются автоматически — это не настраивается). Если не продлить — клиент со временем уходит в «Выбывшие» (через 30 дней после окончания последнего абонемента при отсутствии активных), сам абонемент не создастся.",
    ...
    "- Там же в карточке направления: длительность занятия (мин), «Бесплатное пробное занятие» (галочка) или цена пробного, «Стоимость разового посещения» (списывается с баланса родителя, когда ученика добавляют на конкретное занятие без абонемента; если пусто — берётся цена занятия), цвет и иконка направления.",
    "- Платить ли педагогу за пробные занятия (с 07.07.2026): левое меню → «Настройки» → вкладка «Организация» → «Параметры системы» → «Оплата пробных занятий педагогу», три режима: «Не платить» (по умолчанию), «Только платные» (ЗП начисляется, если у направления задана цена пробного), «Все пробные» (в т.ч. бесплатные). Менять может владелец или управляющий. Режим определяет галочку «Оплата инструктору» у НОВЫХ пробных по умолчанию; на конкретном пробном галочку можно переключить вручную в карточке занятия (это пересчитает начисление, даже если пробное уже отмечено). Смена режима задним числом уже отмеченные пробные НЕ пересчитывает. Ограничение: ЗП начисляется только за пробные, привязанные к занятию группы; за индивидуальное пробное (без группы) начисления нет ни в одном режиме.",
    ...
    "- Перенести пробное занятие (дата/время/педагог/кабинет): «Продажи» в левом меню (блок «CRM») → вкладка «Пробное» → ПКМ по строке → «Изменить». Для пробного в группе меняются дата (по расписанию группы) и сама группа — время определяется занятием группы; для индивидуального пробного — дата, время, педагог и кабинет. Второй способ: «Расписание» → клик по карточке пробного (бейдж «проб») → окно деталей → кнопка «Перенести» (владелец/управляющий/администратор). Старое пробное отменяется, создаётся новое.",
    "- Кампания обзвона: «Обзвон» в левом меню (блок «CRM») → кнопка «Новый обзвон» (откроется диалог «Новая кампания обзвона»)",
    "- Импорт клиентов / миграция базы (CSV/XLSX): левое меню → «Настройки» → вкладка «Персональные» → «Импорт базы» (доступно только владельцу). Кнопки «Импорт» на странице «Клиенты» больше НЕТ. ВАЖНО: у каждой строки файла должен быть заполнен телефон ИЛИ соцсети. Этап 1 помечает строки без обоих контактов «Проверить = да» в промежуточном файле (заполнить или удалить при вычитке), Этап 2 такие строки не пропускает — блокирует загрузку целиком со списком проблемных строк. Это защита от неискомых клиентов и дублей при повторном импорте.",
