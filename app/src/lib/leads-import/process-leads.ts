...
// Кроме сырой выгрузки, обработку Шага 1 можно прогнать и на ЗАПОЛНЕННОМ шаблоне
// («Ребёнок», «Фамилия Имя родителя», «Номер_телефона») — тогда владелец тоже
// получит проверенный файл с пометками. Семантика та же: «ФИО»/«Ребёнок» = ребёнок,
// «Контактное лицо»/«Фамилия Имя родителя» = родитель.
const COLUMN_FIO = ["ФИО", "Фамилия Имя", "Ребёнок", "Ребенок"]
const COLUMN_CONTACT = ["Контактное лицо", "Контактное_лицо", "Фамилия Имя родителя"]
const COLUMN_PHONE = ["Телефон", "телефон", "Номер_телефона", "Номер телефона"]
