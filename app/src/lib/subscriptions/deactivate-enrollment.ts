...
export interface DeactivateEnrollmentInput {
  ...
  /**
   * Явная граница состава (withdrawnAt). Если задана, используется КАК ЕСТЬ,
   * без вывода по последнему платному занятию. Два случая:
   *
   * 1) ОТЛОЖЕННОЕ отчисление (Подход A): ребёнок ходит и платит по факту до
   *    даты X, из расписания выпадает после — передаётся X+1. Вывод по
   *    последнему платному выкинул бы ребёнка из расписания немедленно
   *    (баг: занятия до даты отчисления пропадали сразу при планировании).
   *
   * 2) Штатное ЗАКРЫТИЕ (кроны автозакрытия/пакетов, PATCH closed): месяц
   *    отработан, ребёнок легитимно в составе до конца периода — передаётся
   *    конец периода + 1. Вывод по последнему платному здесь удалял бы
   *    финальные «Уваж. пропуск»/«Перерасчёт» (chargeAmount=0) чисткой хвоста,
   *    а у абонемента со 100% скидкой (платных отметок нет вовсе) обнулял бы
   *    всю историю состава (withdrawnAt=enrolledAt).
   *
 ...
 * И тип не списывает и не платит инструктору; платные посещения, отработки (isMakeup /
