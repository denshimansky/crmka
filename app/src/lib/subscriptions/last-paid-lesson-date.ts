...
// на абонементе нет — списание всё равно записано). Пробные сюда не попадают
// не из-за суммы (платное пробное тоже даёт chargeAmount > 0), а потому что
// у них нет subscriptionId — функция считает строго по конкретному абонементу.
// Pending-заглушки отсеиваются суммой: у них chargeAmount = 0.
//
