...
describe("инструкции модели", () => {
  ...
  it("запрещают вываливать всю карточку в ответ на простой вопрос", () => {
    // На боевой проверке модель на вопрос «когда занятие» добавила и долг, и
    // баланс — родителю это читается как намёк на оплату.
    assert.match(AI_REPLY_SYSTEM, /НЕ перечисляй остальные данные карточки/)
  })
})
