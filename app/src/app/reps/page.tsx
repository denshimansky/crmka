...
const modules: Module[] = [
  {
    ...
    reports: [
      ...
      },
      {
        ...
        data: [
          { what: "Пробное занятие: инструктор, статус, дата посещения", from: "Диалог «Записать на пробное» (выбор инструктора); статус и attendedAt — отметка занятия" },
        ...
        formula: "Конверсия = COUNT(продаж) / COUNT(пробных) × 100%. Пробные — клиенты, посетившие пробное у инструктора. Продажа = первое платное занятие посещено ИЛИ первая оплата внесена (что раньше)",
        status: "ok",
      },
      {
        id: "CRM-15",
      ...
      },
      {
        ...
        data: [
          ...
          { what: "Связи: инструктор, направление, кабинет, филиал", from: "Авто из группы абонемента (/schedule/groups)" },
        ],
        ...
        status: "partial",
        gap: "Нет страницы /settings/withdrawal-reasons и модели WithdrawalReason — поле «причина» сохраняется как UUID без таблицы. Нет флагов «исключить из оттока по направлению / по инструктору» при отчислении",
      },
      {
        id: "CRM-24",
      ...
      {
        id: "CRM-27",
        name: "Конверсия оттока по инструкторам",
        data: [
          { what: "Абонемент: группа → инструктор", from: "Группа задаётся в диалоге «Новый абонемент»; инструктор группы — поле в /schedule/groups (или замена через диалог в карточке группы)" },
          { what: "Активные и выбывшие абонементы у инструктора", from: "Авто" },
        ],
        formula: "% оттока = COUNT(выбывших абонементов) / COUNT(активных абонементов) × 100% по инструктору. При смене инструктора в середине месяца активные считаются обоим — сумма по инструкторам может быть больше оттока по направлениям",
        status: "partial",
        gap: "Нет флага «исключить из оттока по инструктору» при отчислении абонемента",
      },
      {
        id: "CRM-28",
      ...
      {
        id: "CRM-36",
        name: "Сводный по абонементам в разрезе инструкторов",
        data: [
          { what: "Абонемент: группа → инструктор", from: "Группа — диалог «Новый абонемент»; инструктор — поле группы в /schedule/groups" },
          { what: "Посещения со списаниями", from: "Отметка явок в карточке занятия" },
        ],
        formula: "По инструктору: Активные абонементы (≥1 списание), Новые (продажа И первое занятие с явкой в месяце), Выбывшие (нет следующего абонемента), Активные на конец месяца",
        status: "ok",
      },
      {
        id: "CRM-37",
        name: "Сколько денег приносит инструктор",
        data: [
          ...
          { what: "Расход: переменные (канцтовары)", from: "Статья расхода «Канцтовары» в /finance/expenses (склад информационный, на финрез не влияет)" },
          { what: "Расход: постоянные (распределяются пропорционально)", from: "Диалог «Новый расход» в /finance/expenses" },
        ],
        formula: "Прибыльность инструктора = Выручка − Зарплата − Переменные расходы (канцтовары пропорц. по занятиям) − Доля постоянных расходов. % от общего дохода = Прибыльность инструктора / Общая чистая прибыль",
        status: "ok",
      },
      {
        id: "CRM-38",
    ...
    reports: [
      ...
      },
      {
        ...
        data: [
          ...
          { what: "Сетка «строка-ученик × колонки-дни месяца»", from: "/lessons/attendance — фильтры филиал/кабинет/направление/инструктор/группа, колонка «План» (число занятий) и «К оплате» (Subscription.balance)" },
        ],
    ...
    reports: [
      ...
      },
      {
        ...
        data: [
          ...
          { what: "Абонемент, скидка, группа, инструктор", from: "Авто из связей при оплате" },
        ],
      ...
      },
      {
        ...
        data: [
          ...
          { what: "Переменные расходы (среднее)", from: "Среднее по статье «Канцтовары» в /finance/expenses (склад на финрез не влияет)" },
          { what: "Плановые расходы по статьям", from: "Диалог в /finance/planned-expenses" },
        ],
        formula: "Прогноз прибыли = Сумма абонементов − Прогноз ЗП инструкторов − Прогноз переменных расходов − Прогноз постоянных платежей. Прогноз ЗП — авто (расписание × ставки). Перемен. — среднее по статье канцтоваров. Постоян. — вручную (план)",
        status: "ok",
      },
      {
        id: "FIN-14",
      ...
      },
      {
        ...
        formula: "К оплате = План − Оплачено по каждой статье ДДС. ЗП инструкторов — авто из ставок (не вручную)",
        status: "ok",
      },
      {
        id: "FIN-25",
  ...
  },
  {
    ...
    reports: [
      ...
      {
        id: "SAL-16",
        name: "Часы инструкторов по дням",
        data: [
  ...
  },
  {
    ...
    reports: [
      {
        id: "INV-06",
        name: "Остатки по складу, филиалам и кабинетам",
        data: [
          { what: "Остатки на общем складе", from: "Авто (WarehouseBalance — после внесения/перемещения)" },
          { what: "Остатки в филиале", from: "Авто (StockBalance — после перемещения со склада)" },
          { what: "Остатки в кабинете", from: "Авто (RoomBalance — после перемещения и списания)" },
          { what: "Товар: название, единица, цена", from: "Диалог «Внести на склад» в /stock" },
        ],
        formula: "Остаток = SUM(приход) − SUM(расход) на дату. Сумма = накопленная себестоимость (totalCost) локации. Итого по дням = (приходы − расходы) за месяц",
        status: "ok",
      },
    ],
  },
        ...
        <CardContent>
          <ol className="ml-4 list-decimal space-y-3 text-sm">
            ...
            <li>
              ...
              <span className="text-muted-foreground">
                {" "}— чекбоксы «отток по направлению» и «отток по инструктору» в диалоге отчисления абонемента.
