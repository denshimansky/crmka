...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Инструктор занятия копируется из группы при генерации, поэтому смену инструктора
  // распространяем на запланированные занятия начиная с ЗАВТРА. Сегодняшние не
  // трогаем: утреннее занятие могло быть уже проведено старым инструктором, но ещё
  ...
  // по ставке прежнего инструктора; заранее проставленные отметки без начислений
