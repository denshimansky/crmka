...
export async function PATCH(
  ...
  // v1-restrict: убрать занятие из плана можно, только пока по нему НЕТ реальной
  // отметки этого абонемента — любой, а не только списывающей.
  //
  // Раньше условие включало chargeAmount > 0, и несписывающая отметка
  // («Перерасчёт», «Уваж. пропуск», «Не был») оставалась висеть на занятии,
  // выпавшем из плана. Пакет его больше не покрывает → ребёнок пропадает из
  // состава, а старая отметка всплывает отдельной «разовой» строкой; стоит снять
  // её («Не отмечен») — и ребёнок исчезает с занятия совсем. Кейс Жуковой
  // (17.08.2026, ДЦ Первое Слово): два «Перерасчёта» осиротели после свопа плана
  // на 21.08. Комментарий над этой проверкой всегда говорил «только НЕотмеченные»
  // — код просто не соответствовал правилу.
  //
  // Списывающая отметка блокирует по прежней причине (нужен откат списания, его
  // точка — снятие отметки на самом занятии), несписывающая — чтобы не плодить
  // осиротевшие строки. Действие для оператора одинаковое: снять отметку.
  if (removed.length > 0) {
    ...
    if (marked) {
      ...
      return NextResponse.json(
        {
          error: `На занятии ${d} ${marked.lesson.startTime} уже стоит отметка «${marked.attendanceType.name}». Сначала снимите её (в карточке занятия — «Не отмечен»), затем меняйте план пакета.`,
        },
        { status: 409 },
      )
    }
