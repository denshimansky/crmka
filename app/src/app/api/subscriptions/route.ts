...
export async function POST(req: NextRequest) {
  ...
  const subscription = await db.$transaction(async (tx) => {
    ...
    // создать/реактивировать GroupEnrollment ДО recalcClientDiscounts.
    // enrolledAt = startDate (а не now): при выборе даты задним числом ребёнок
    // появится в таблицах посещений прошлых занятий месяца. Детали и правила —
    // в ensure-enrollment.ts (общая логика с массовой выпиской).
