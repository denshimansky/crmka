...
export async function POST(req: NextRequest) {
  ...
  const branch = await db.branch.findFirst({
    where: { id: parsed.data.branchId, tenantId, deletedAt: null },
    select: { id: true },
  })
  if (!branch) return NextResponse.json({ error: "Филиал не найден" }, { status: 404 })

  // Создаём по строке на каждый тип бонуса с amount=NULL = «использовать
  // глобальную ставку», пока пользователь не введёт значение в инпут.
  for (const bonusType of BONUS_TYPES) {
    await upsertScopeBonus(tenantId, branch.id, null, bonusType, null)
  }
