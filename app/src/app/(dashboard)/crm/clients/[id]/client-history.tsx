...
const FILTERS: FilterDef[] = [
  ...
  { key: "att", label: "Посещения", kinds: ["attendance_present", "attendance_absent", "attendance_other", "lesson_note"] },
  { key: "status", label: "Статусы", kinds: ["status_change"] },
  { key: "discount", label: "Скидки", kinds: ["template_discount_removed", "discount_mode_changed"] },
]
...
> = {
  ...
  // Комментарий оператора к занятию из реестра «Пропуски». В событие посещения
  // он встраивается строкой, а отдельным событием приходит, когда отметки нет
  // (неотмеченное занятие) или занятие уже удалено.
  lesson_note: { icon: StickyNote, color: "text-gray-600", bg: "bg-gray-100 dark:bg-gray-800" },
  ...
  discount_mode_changed: { icon: Percent, color: "text-violet-700", bg: "bg-violet-100 dark:bg-violet-900/30" },
