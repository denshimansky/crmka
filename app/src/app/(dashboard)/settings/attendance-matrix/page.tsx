...
// Коды, у которых «Разрешить отчисление» залочено на false. Дублирует
// WITHDRAWAL_LOCKED_CODES из lib/subscriptions/withdrawal-block: тот модуль
// серверный (тянет Prisma), а эта страница — "use client".
const WITHDRAWAL_LOCKED_CODES = new Set(["makeup_scheduled", "no_show"])
...
const SYSTEM_TYPE_DESCRIPTIONS: Record<string, string> = {
  ...
  no_show:
    "Промежуточный статус: ребёнок не пришёл, причина ещё не выбрана. Пока он стоит, абонемент отчислить нельзя — сначала уточните причину («Уваж. пропуск», «Прогул» или назначьте отработку).",
}
              ...
              <TableBody>
                {types.map((t) => (
                  <TableRow key={t.id} className={!t.isActive ? "opacity-50" : ""}>
                    ...
                    <TableCell className="text-center">
                      {WITHDRAWAL_LOCKED_CODES.has(t.code) ? (
                        // Запрет зашит в систему, галочку не поставить:
                        //  - «Назначена отработка» — незакрытое обязательство;
                        //  - «Не был» — причина пропуска ещё не выбрана, а на
                        //    отчисленном абонементе её уже никто не переставит.
                        <span
                          ...
                          title={
                            t.code === "makeup_scheduled"
                              ? "Нельзя разрешить: назначенная отработка блокирует отчисление, пока её не проведут или не отменят"
                              : "Нельзя разрешить: «Не был» — промежуточный статус. Сначала уточните причину пропуска (Уваж. пропуск, Прогул или отработка)"
                          }
                        />
                      ) : (
