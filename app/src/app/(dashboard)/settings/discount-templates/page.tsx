      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            ...
            {editTemplate?.kind === "second_subscription" && (
              <p className="text-xs text-muted-foreground">
                Автоматическая скидка «за второй и следующие»: при двух и более
                абонементах родителя в одном месяце БЕЗ ручной скидки применяется ко
                всем таким, кроме самого дорогого. После включения действует на
                абонементы со следующего месяца; текущий месяц не пересчитывается.
              </p>
            )}
