        ...
        <CardContent>
          ...
          {uncoveredStudents.length > 0 && (
            <div className="mt-4 rounded-md border border-dashed p-3 text-xs text-muted-foreground">
              ...
              эту дату у ребёнка нет абонемента по направлению группы, который
              покрывал бы это занятие: на месяц занятия абонемент ещё не выписан
              (на каждый месяц выписывается новый — абонемент за прошлый месяц
              следующий не покрывает), либо он начинается позже даты занятия, либо
              абонемент отчислён, либо (у пакета) это занятие не входит в план,
              пакет истёк или в нём не осталось занятий. Отметку поставить
              нельзя — списывать не с чего.
              <br />
              Оплата значения не имеет: выписанный, но ещё не оплаченный абонемент
              (статус «Ожидаем оплату») покрытие даёт — ребёнка можно отмечать с
              первого занятия месяца, не дожидаясь оплаты.
              <br />
              Что можно сделать: выписать или продлить абонемент на месяц занятия
              (для пакета — добавить это занятие в план) — строка станет обычной;
              либо «Добавить разовым» — оплата спишется с баланса родителя при
              отметке «Был». Строка показана, чтобы ребёнок не пропадал из занятия
              молча: он зачислен в группу, но абонемента на это занятие пока нет.
            </div>
          )}
        </CardContent>
      </Card>
