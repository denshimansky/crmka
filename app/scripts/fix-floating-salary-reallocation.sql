...
--     «Был» и оплачиваемый «Прогул» двигают порог, «Не был»/«УП» — нет;
--   - floating: брекет по числу уникальных учеников с оплачиваемой отметкой
--     (ВКЛЮЧАЯ пробные — пробный сидел на занятии и двигал порог и в calcPay);
--   - вся ставка на первой НЕ-пробной оплачиваемой отметке;
...
--   FROM _fix_salary_realloc_backup_20260710 b WHERE a.id = b.attendance_id;
...
marks AS (
  ...
  SELECT r.lesson_id, r.tenant_id, r.scheme, r.rate_per_lesson, r.gsr_id, r.sr_id, r.emp_id, r.date,
         ...
         a.instructor_pay_enabled AS pay_on,
...
counts AS (
  SELECT lesson_id,
         COUNT(DISTINCT (client_id::text || ':' || COALESCE(ward_id::text, '')))
           FILTER (WHERE pay_on) AS n_payable,               -- включая пробных
         ...
         CASE
           ...
           ELSE COALESCE((
             ...
             WHERE ((m.gsr_id IS NOT NULL AND sb.group_salary_rate_id = m.gsr_id)
               ...
               AND sb.min_students <= c.n_payable
...
carriers AS (
  -- Носитель — первая НЕ-пробная оплачиваемая отметка по времени
  SELECT DISTINCT ON (m.lesson_id) m.lesson_id, m.attendance_id AS carrier_id
  FROM marks m
  WHERE m.pay_on AND NOT m.is_trial
...
CREATE TABLE IF NOT EXISTS _fix_salary_realloc_backup_20260710 AS
...
-- Сверка от живых данных: инвариант «не больше одной ставки на не-пробных отметках»
