-- =============================================================================
-- ПЕРЕСОЗДАНИЕ ТЕСТОВОЙ ДЕМО-БАЗЫ (17.07.2026)
--
...
--     -P pager=off -v ON_ERROR_STOP=1" < app/scripts/demo-clone-dream-to-sokolova.sql
-- Откат после COMMIT: восстановить из бэкапа pre-demo-clone-20260717.dump
-- =============================================================================
...
$fn$ SELECT md5(u::text || 'demo-clone-2026-07-17')::uuid $fn$;
...
DECLARE
  ...
  kept_emps constant uuid[] := ARRAY[
    'e7c3559b-53e0-4f02-b1fe-1a0f48b7fb3c', -- owner   / Соколова Татьяна
    'e4e27dd2-872c-4716-8d8d-f36a70fc6379', -- manager / Белов
    'b02f3861-b60d-4cdb-8c53-f66509f76edc', -- admin   / Козлова
    '24adf840-5d3a-47a1-ae0a-c2eff82c894a', -- admin2  / Морозова
    'aafbc538-503f-45d2-bd5c-c0e27dfd3085', -- cyberj1nn / Малафеев
    '4f9eeebb-ffa2-4f60-a0b9-95f7bc7486ba'  -- viewer  / Иванов
  ]::uuid[];
  r record; c record;
...
BEGIN
  ...
  FOR r IN SELECT DISTINCT table_name FROM information_schema.columns
           WHERE table_schema = 'public' AND column_name = 'tenant_id'
             AND table_name <> ALL(skip_tables) LOOP
