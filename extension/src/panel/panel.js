...
async function renderForChat() {
  ...
  if (!chat || chat.unsupported) {
    // Групповой чат сюда попадает СО СВОИМ chatId — и дальше не идёт: ни
    // резолва, ни привязки, ни заливки. Третий из четырёх рубежей (адаптер,
    // service worker, панель, сервер).
    showNoChatReason(chat)
    return
  }
...
/**
 * Почему панель не показывает клиента. Причины разные, и действия человека
 * тоже разные, поэтому не отделываемся общим «откройте чат».
 *
 * @param {ChatContext|null} [chat] Чат, который панель как раз решила не
 *   обслуживать (групповой). Передаётся снимком из renderForChat: пока шли
 *   запросы, `state.chat` мог смениться.
 */
/**
 * Хосты мессенджеров, которые обслуживает расширение, — человекочитаемым
 * перечислением («web.telegram.org, web.max.ru или web.whatsapp.com»).
 *
 * Источник правды — манифест: там же объявлены content_scripts, и именно их
 * набор определяет, где панель реально работает. Отдельный список в тексте
 * подсказки расходился бы с действительностью ровно в тот момент, когда
 * добавляется канал, — и панель звала бы открыть не тот сайт поверх уже
 * открытого нужного.
 *
 * @returns {string}
 */
function messengerHostsText() {
  ...
  const list = [...hosts]
  if (!list.length) return "веб-версию мессенджера"
  if (list.length === 1) return list[0]
  return `${list.slice(0, -1).join(", ")} или ${list[list.length - 1]}`
}

function showNoChatReason(chat) {
  showStatus("")
  el.noChat.hidden = false
  const tab = state.tab

  if (chat?.unsupported === "group") {
    // Не поломка, а решение: переписку чата, за которым стоит не один человек,
    // в карточку одного клиента заливать нельзя, и убрать её оттуда потом нечем.
    //
    // Формулировка по каналу: во ВКонтакте таких чатов два вида и называются
    // они иначе («беседа», «диалог с сообществом»). Общий текст про «группу,
    // рассылку или канал» человек там прочитал бы как «панель меня не поняла».
    el.noChatHint.textContent =
      chat.channel === "vk"
        ? "Это беседа или диалог с сообществом. Панель работает с перепиской один на один: откройте диалог с родителем."
        : "Это не личный чат (группа, рассылка или канал). Панель работает с перепиской один на один: откройте диалог с родителем."
    return
  }

  if (chat?.unsupported === "no-id") {
    // Только WhatsApp. Опознать собеседника там можно лишь по номеру телефона,
    // а номер виден только у контактов, НЕ сохранённых в телефонной книге: у
    // сохранённых WhatsApp показывает имя, и больше в разметке нет ничего.
    // Человеку это надо сказать прямо — иначе он будет искать поломку там, где
    // её нет, и придумывать, что «не так» именно с этим чатом.
    el.noChatHint.textContent =
      "WhatsApp не показывает номер этого собеседника — он сохранён у вас в контактах телефона, и панель не может понять, кто это. Работает с теми, чей номер виден в заголовке чата."
    return
  }

  if (!tab?.onMessenger) {
    // Список мессенджеров берём ИЗ МАНИФЕСТА, а не пишем руками: он тут уже
    // третий, и перечисление в тексте молча устаревало бы при добавлении
    // канала — панель звала бы открыть не тот сайт поверх открытого нужного.
    el.noChatHint.textContent =
      `Панель работает поверх веб-мессенджера. Откройте в этой вкладке ${messengerHostsText()} и вернитесь сюда.`
    return
  }

  if (!tab.contentAlive) {
    // Обычный случай сразу после установки: вкладка мессенджера была открыта
