...
function showNoChatReason(chat) {
  ...
  if (chat?.unsupported === "no-messages") {
    // Только WhatsApp: там идентификатор чата берётся из разметки самих
    // сообщений, и в пустом диалоге его взять неоткуда. Человеку это надо
    // сказать прямо — иначе он будет искать поломку там, где её нет.
    el.noChatHint.textContent =
      "Чат открыт, но в нём пока нет сообщений — по ним панель и узнаёт собеседника. Напишите первое сообщение или дождитесь ответа."
    return
  }
