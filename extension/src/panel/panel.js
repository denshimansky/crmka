...
function showNoChatReason(chat) {
  ...
  if (chat?.unsupported === "group") {
    // Не поломка, а решение: групповую переписку в карточку одного человека
    // заливать нельзя, и убрать её оттуда потом нечем.
    el.noChatHint.textContent =
      "Это групповой чат. Панель работает с личной перепиской: откройте диалог с родителем один на один."
    return
  }

  if (!tab?.onMessenger) {
    // Мессенджеров уже два, и называть один из них — значит сбивать с толку
    // ровно половину сотрудников.
    el.noChatHint.textContent =
      "Панель работает поверх веб-мессенджера. Откройте в этой вкладке web.telegram.org или web.max.ru и вернитесь сюда."
    return
  }
