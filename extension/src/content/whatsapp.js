...
;(() => {
  ...
  function collectVisibleMessages() {
    ...
    // Чат нечем опознать — ни номера, ни единого сообщения. Собирать нечего и
    // незачем: сервер такую заливку всё равно отклонит («чат не привязан»).
    if (!chat.chatId && !chat.messageIds?.length) return []
  ...
  function readChat() {
    ...
    // Групповой чат. Признак косвенный, но надёжный: в переписке один на один
    // подпись входящего сообщения совпадает с заголовком чата — и то и другое
    // берётся из имени одного и того же контакта. Разошлись — значит пишет не
    // тот, чьё имя в шапке, то есть это группа.
    //
    // Прямого признака у нас нет: JID-ов в разметке не осталось, а по ним
    // группа отличалась бы суффиксом «@g.us». Ошибка тут необратима — вся
    // групповая переписка чужих родителей уехала бы в карточку одного человека.
    if (looksLikeGroup(root, title)) {
      return {
        channel: "whatsapp",
        chatId: "",
        localKey: `whatsapp-group:${title ?? ""}`,
        altIds: [],
        messageIds: [],
        peerSource: "подписи сообщений не совпадают с заголовком — похоже на группу",
        title,
        phone: null,
        unsupported: "group",
      }
    ...
    // Идентификаторы видимых сообщений — примета чата. По ним сервер узнаёт
    // диалог, у которого нет собственного идентификатора: сообщение
    // принадлежит ровно одному чату, и эта примета переживает и переименование
    // контакта, и совпадение имён двух клиентов.
    const messageIds = readVisibleMessageIds(root)

    // Номер есть — синтезируем канонический вид JID. Так серверу не нужно
    // ничего знать про особенности WhatsApp Web: «79001234567@c.us» он
    // нормализует в тот же телефонный ключ, что и всегда, и матч по номеру
    // работает как в любом другом месте CRM.
    const chatId = phone ? `${phone}@c.us` : ""

    return {
      channel: "whatsapp",
      chatId,
      // ЛОКАЛЬНЫЙ ключ — только для сравнения «тот же диалог или уже другой»
      // внутри расширения. На сервер он не уходит НИКОГДА: заголовок чата
      // идентификатором быть не может, два тёзки дали бы одну карточку. Но для
      // локального сравнения он годится, а цена промаха здесь мала — сервер
      // всё равно сверяет, кому принадлежат присланные сообщения.
      localKey: chatId || `whatsapp-title:${title ?? ""}`,
      altIds: chatId ? [chatId] : [],
      messageIds,
      peerSource: titlePhone
        ? "номер из заголовка чата"
        : labelPhone
          ? "номер из подписи сообщения"
          : messageIds.length
            ? "по сообщениям чата"
            : "нечем опознать: ни номера, ни сообщений",
      title,
      phone,
      // Чат без номера и без единого сообщения опознать нечем: примету взять
      // не из чего. Такое бывает в пустом диалоге у сохранённого контакта.
      unsupported: chatId || messageIds.length ? null : "no-id",
    }
  }

  /**
   * Идентификаторы видимых сообщений — хвост ленты.
   *
   * Берём только настоящие сообщения (со служебных строк примету снимать
   * незачем) и только последние: этого достаточно, чтобы узнать чат, а грузить
   * сервер сотнями идентификаторов при каждом опросе не надо.
   *
   * @param {HTMLElement} root
   * @returns {string[]}
   */
  function readVisibleMessageIds(root) {
    if (!waJid) return []
    const rows = root.querySelectorAll(SEL.row)
    /** @type {string[]} */
    const out = []
    for (let i = rows.length - 1; i >= 0 && out.length < COLLECT_LIMIT; i--) {
      const row = /** @type {HTMLElement} */ (rows[i])
      if (!row.querySelector(SEL.bubble)) continue
      const key = waJid.parseMessageKey(row.getAttribute("data-id"))
      if (key?.messageId) out.push(key.messageId)
    }
    return out.reverse()
  }

  /**
   * Похож ли открытый чат на групповой.
   *
   * В переписке один на один подпись входящего сообщения совпадает с
   * заголовком чата: и то и другое WhatsApp берёт из имени одного контакта. В
   * группе подписи принадлежат разным участникам и с заголовком (названием
   * группы) не совпадают.
   *
   * Проверяем ТОЛЬКО входящие, и определяем их ХВОСТОМ, а не readDirection.
   * Это принципиально: readDirection сама сравнивает подпись с заголовком, и
   * если бы мы спросили её, то в группе каждое входящее (подпись участника ≠
   * заголовок группы) она объявила бы исходящим — а значит признак группы не
   * сработал бы НИ РАЗУ. Хвост же — прямое свидетельство, ни от чего не
   * зависящее.
   *
   * Заведомо осторожно: если ни одного входящего с хвостом на экране нет
   * (например, в чате одни голосовые от нас), группой чат НЕ считаем — иначе
   * панель отказывалась бы работать там, где всё в порядке. Прямого признака
   * группы в разметке не осталось вовсе, и это принятое ограничение.
   *
   * @param {HTMLElement} root
   * @param {string|null} title
   * @returns {boolean}
   */
  function looksLikeGroup(root, title) {
    ...
    for (let i = scanFrom; i < rows.length; i++) {
      const row = /** @type {HTMLElement} */ (rows[i])
      if (!row.querySelector(SEL.bubble)) continue
      if (!row.querySelector(SEL.tailIn)) continue
      const label = readAuthorLabel(row)
      if (!label) continue
      if (label.replace(/:$/, "").trim() !== chatTitle) return true
    }
    return false
  }
