...
const DEFAULT_SETTINGS = {
  // Боевая база живёт на msk1 (Timeweb), и это ЕДИНСТВЕННЫЙ адрес CRM в
  // host_permissions (решение 01.09.2026: каждый лишний домен в карточке стора
  // приходится объяснять ревьюеру). Следствие: когда прод переключат на
  // app.umnayacrm.ru, одной сменой адреса в панели уже не обойтись — нужна
  // новая версия в сторе с добавленным доменом, а Chrome на добавление
  // host-права отключает расширение до повторного согласия пользователя.
...
//
// БЕЗ ПРАВА "tabs" (мы его не просим — см. манифест) changeInfo.url приходит
// только для вкладок, попадающих в host_permissions. То есть уход С мессенджера
// НА посторонний сайт этот обработчик уже не увидит: адрес нового сайта нам не
// показывают. Это осознанный размен, а не недосмотр — чистку страхует
// MSG_GET_STATE, который на каждом обращении панели проверяет, мессенджер ли
// сейчас в активной вкладке, и выбрасывает чат сам. Обработчик оставлен: там,
// где адрес виден (переход внутри мессенджера, возврат на него), он срабатывает
// раньше и точнее.
