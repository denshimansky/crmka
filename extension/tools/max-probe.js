...
;(() => {
  ...
  const api = {
    ...
    async tryInsert(способ = "paste", text = "строка один\nстрока два") {
      ...
      // Осторожно с вычислением «что вставилось»: у ПУСТОГО редактора Lexical
      // innerText равен «\n» (он рисует пустой абзац), а не пустой строке.
      // Наивный slice по длине прошлого текста съедал первый символ — в отчёте
      // выходило «трока один» вместо «строка один». Это была ошибка ИЗМЕРЕНИЯ,
      // а не вставки: в поле ввода текст стоял правильный.
      ...
      return {
        ...
        // Двойной перенос означает, что Lexical сделал АБЗАЦЫ, а не мягкие
        // переносы: innerText склеивает соседние блочные узлы через «\n\n».
        // На экране выглядит одинаково, но в отправленном сообщении между
        // строками может оказаться пустая строка.
    ...
    /**
     * Виртуализируется ли список сообщений.
     *
     * ВАЖНО про толкование: равенство «до» и «после» само по себе НИЧЕГО не
     * доказывает. В коротком чате сверху просто нечего подгружать, и счётчик не
     * меняется ровно так же, как при виртуализации. Первая версия этой проверки
     * в таком случае бодро отвечала «похоже на виртуализацию» — вывод был
     * неверный. Поэтому теперь возвращаем ФАКТЫ (была ли вообще прокрутка,
     * сколько узлов, изменилась ли высота), а вердикт даём только когда он
     * обоснован. Запускать надо в чате с ДЛИННОЙ историей.
     */
    scrollCheck() {
      ...
      return new Promise((resolve) =>
        setTimeout(() => {
          ...
          resolve({
            ...
            прокруткаБыла,
            вывод: !прокруткаБыла
              ? "НЕИНФОРМАТИВНО: прокручивать нечего, история целиком на экране. Нужен чат с длинной перепиской"
              : after > before
                ? "узлы накапливаются — виртуализации нет"
                : after < before
                  ? "виртуализация: часть узлов выгружена"
                  : "история докручена до конца и не выросла — вероятно, виртуализации нет",
          })
        }, 1500),
      )
    },
